    ...
    D: LegacyMemoryRegion,
{
    ...
    let physical_memory_offset = if let Some(mapping) = config.mappings.physical_memory {
        ...
        log::info!("  max_phys = {:#x}", max_phys.as_u64());
        
        // Cap to 4GB to avoid huge MMIO regions blowing up the frame allocator.
        // Kernel can still access all RAM via physical_memory_offset.
        let cap_phys = max_phys.min(PhysAddr::new(0x1_0000_0000));
        let end_frame: PhysFrame<Size2MiB> = PhysFrame::containing_address(cap_phys - 1u64);

        let size = cap_phys.as_u64();
        ...
        log::info!("  phys_mem_offset = {:#x}", offset.as_u64());
        ...
        log::info!("  mapped {} x 2MiB frames", count);
